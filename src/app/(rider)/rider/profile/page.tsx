...
type Profile = { email: string; fullName: string; phone: string; ndisId: string };
...
export default function RiderProfilePage() {
  ...
  useEffect(() => {
    ...
    apiJson<Profile>("/rider/profile", undefined, session.accessToken)
      .then((p) => {
        setProfile(p);
  ...
  async function saveProfile() {
    ...
    setMsg("");
    try {
      await apiJson("/rider/profile", {
        method: "PATCH",
        body: JSON.stringify({ fullName, phone, ndisId }),
      }, session.accessToken);
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <Card>
          ...
          <div className="mt-3 grid gap-3 md:grid-cols-2">
            ...
            <Input placeholder="Plan Manager Email" className="md:col-span-2" />
          </div>
          {loading ? (
