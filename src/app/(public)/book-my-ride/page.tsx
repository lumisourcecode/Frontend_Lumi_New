          ...
          {step === 2 ? (
            <div className="grid gap-3 md:grid-cols-2">
              <div>
                ...
                <PlacesAutocomplete
                  value={form.pickup}
                  onChange={(v, place) => setForm((p) => ({
                    ...p,
                    pickup: v,
                    pickupCoords: place?.lat != null && place?.lng != null ? { lat: place.lat, lng: place.lng } : null,
                  }))}
                  placeholder="Enter pickup address in Australia"
                />
              </div>
              <div>
                ...
                <PlacesAutocomplete
                  ...
                  onChange={(v, place) => setForm((p) => ({
                    ...p,
                    destination: v,
                    dropoffCoords: place?.lat != null && place?.lng != null ? { lat: place.lat, lng: place.lng } : null,
                  }))}
            ...
            {step < 4 ? (
              ...
              <Button
                ...
                onClick={async () => {
                  ...
                  try {
                    await apiJson("/rider/bookings", {
                      method: "POST",
                      body: JSON.stringify({
                        pickup: form.pickup || "Pickup location",
                        dropoff: form.destination || "Destination",
                        scheduledAt,
                        mobilityNeeds: form.mobility,
                        notes: form.notes,
                      }),
                    }, session.accessToken);
                    setSubmitted(true);
